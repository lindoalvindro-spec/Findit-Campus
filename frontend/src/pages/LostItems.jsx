...
const LostItems = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const { data, error } = await apiClient.get('/api/items?status=lost');
      ...
      <main className="flex-grow w-full max-w-[1280px] mx-auto px-margin-mobile md:px-margin-desktop py-xl">
        ...
        <div className="mb-lg relative rounded-3xl overflow-hidden bg-gradient-to-b from-surface-container-low via-surface-container-low to-surface border border-outline-variant/60 p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
          <div className="relative z-10">
            <span className="font-label-md text-label-md text-primary tracking-widest uppercase mb-2 block font-semibold">Cari & Temukan</span>
            <h1 className="font-headline-xl text-headline-xl text-on-surface mb-4 font-bold leading-tight">
              Daftar <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-surface-tint to-[#2563eb]">Barang Hilang</span>
            </h1>
            <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl leading-relaxed">
              Bantu temukan barang-barang yang dilaporkan hilang di area kampus. Partisipasi Anda sangat berarti dalam menolong sesama mahasiswa.
            </p>
          </div>
          ...
          <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
            <motion.div 
              ...
              className="absolute -top-10 -left-10 w-72 h-72 bg-primary/15 rounded-full filter blur-3xl"
            ></motion.div>
            <motion.div 
              animate={{ 
                x: [0, -20, 20, 0],
              ...
              className="absolute -bottom-10 right-10 w-72 h-72 bg-primary-fixed-dim/20 rounded-full filter blur-3xl"
            ></motion.div>
        ...
        <div className="bg-surface/90 backdrop-blur-md border border-outline-variant/60 rounded-3xl p-4 shadow-[0_8px_30px_rgb(0,0,0,0.03)] mb-lg flex flex-col md:flex-row gap-4 items-center">
          <div className="w-full md:flex-1 relative flex items-center bg-surface-container-low rounded-full px-4 py-2 border border-outline-variant/40 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary-fixed transition-all">
          ...
          <div className="flex w-full md:w-auto gap-3 items-center">
            <div className="relative w-full md:w-56 bg-surface-container-low rounded-full px-4 py-2 border border-outline-variant/40 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary-fixed transition-all flex items-center">
              <select 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-gutter mb-xl">
          {loading ? (
            ...
            paginatedItems.map((item) => (
              <Link key={item.id} to={`/item-detail?id=${item.id}`} className="bg-surface rounded-2xl border border-outline-variant/60 overflow-hidden shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 flex flex-col h-full group">
                <div className="h-48 bg-surface-container flex items-center justify-center relative overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 bg-error-container text-on-error-container px-3 py-1 rounded-full font-label-sm text-[11px] font-semibold z-10 shadow-sm border border-error/10">
                    Hilang
                  </div>
                  {item.campus && (
